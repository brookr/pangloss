...
class AgentRunner {
  ...
  async cloneRepository() {
    return new Promise((resolve, reject) => {
      // Configure git credentials for GitHub token
      const authUrl = this.repoUrl.replace('https://github.com/', `https://${this.githubToken}@github.com/`);
      
      const gitProcess = spawn('git', [
        'clone', authUrl, this.workspaceDir
      ]);
      ...
      gitProcess.on('close', (code) => {
        if (code === 0) {
          ...
          reject(new Error(`Failed to clone repository: ${stderr || 'Unknown error'}`));
        }
      });
    });
  }
...
Please implement the requested feature by making the necessary code changes. Work directly in the codebase and make all required modifications.`;
  ...
  async commitAndPush() {
    // Configure git user
    await this.runCommand(['git', 'config', 'user.email', 'agent@cs.coach']);
    await this.runCommand(['git', 'config', 'user.name', 'Pangloss Agent']);
    ...
    const authUrl = this.repoUrl.replace('https://github.com/', `https://${this.githubToken}@github.com/`);
    await this.runCommand(['git', 'remote', 'set-url', 'origin', authUrl]);
